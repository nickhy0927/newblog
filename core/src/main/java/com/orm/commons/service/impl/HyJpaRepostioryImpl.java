...
@Repository
public class HyJpaRepostioryImpl<T, ID extends Serializable> extends SimpleJpaRepository<T, ID> implements HyJpaRepostiory<T, ID> {

    private Class<T> entityClass;
    private EntityManager entityManager;

    public HyJpaRepostioryImpl(Class<T> domainClass, EntityManager entityManager) {
    ...
    public Page<T> queryByCriteria(Criteria paramCriteria, Pageable pageable) {
        ...
        if (pageable.getSort() != null) {
            for (it = pageable.getSort().iterator(); it.hasNext(); ) {
                Sort.Order order = it.next();
                if (order.getDirection().equals(Sort.Direction.DESC)) {
                    paramCriteria.addOrder(Order.desc(order.getProperty()));
                } else {
                    paramCriteria.addOrder(Order.asc(order.getProperty()));
                }
            }
        }
        Page<T> page = new PageImpl<T>(paramCriteria.list(), pageable, total);
        return page;
    }
}
