...
public abstract class DefaultAbstractService<E, ID extends Serializable> implements BaseService<E, ID> {
	...
	@Override
	public PagerInfo<E> queryPagerInfoByMap(Map<String, Object> map, com.iss.util.PageSupport support, Sort sort)
			throws ServiceException {
		try {
			int currentPage = support.getCurrentPage() - 1 >= 0 ? support.getCurrentPage() - 1 : 0;
			PageSupport pageable = new PageSupport(currentPage, support.getPageSize(), sort);
			Page<E> page = queryPageByMap(map, pageable);
			support.setTotalRecord(queryByMap(map).size());
			PagerInfo<E> pagerInfo = new PagerInfo<>(support, page.getContent());
			return pagerInfo;
		} catch (ServiceException e) {
			e.printStackTrace();
			throw new ServiceException("查询分页出现异常", e);
		}
	}
	...
	public ObjectTools<E> queryPageByMap(Map<String, Object> map, String currentPage, Sort sort, Pager pager)
		...
		int currPage = pager.getCurrentPage() - 1 >= 0 ? pager.getCurrentPage() - 1 : 0;
		PageSupport pageable = new PageSupport(currPage, pager.getPageSize(), sort);
		Page<E> pageInfo = queryPageByMap(map, pageable);
		ObjectTools<E> object = new ObjectTools<E>();
		object.setEntities(pageInfo.getContent());
		object.setPager(pager);
